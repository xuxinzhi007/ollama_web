// UI 渲染与非聊天交互（toast/列表/编辑器/面板等）
// 注意：本项目采用传统多脚本加载（非 ESM），本文件依赖 state/utils/storage/api。

// ==========================================
// 将 UI 控制函数挂载到 window（与原 app.js 保持一致）
// ==========================================
...
// 兼容：loadModels() 会调用 showWelcomeScreen()
function showWelcomeScreen() {
    window.backToHome(true);
}
...
window.saveAgent = async function() {
    ...
    // 生成模型名
    const modelName = editingAgent ? editingAgent.modelName : displayName.toLowerCase();
